...
function analyzeComplexity() {
  ...
  if (!fs.existsSync(COMPLEXITY_REPORT)) {
    console.error(`Complexity report not found at ${COMPLEXITY_REPORT}`);
    console.error('Run npm run complexity first');
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(`Error reading complexity report: ${error.message}`);
    process.exit(1);
  }
...
function generateImprovementOpportunities(complexityData) {
  ...
  opportunities.sort((a, b) => {
    if (a.impact === b.impact) {
      const complexityA = parseInt(a.description.match(/complexity of (\d+)/)[1], 10);
      const complexityB = parseInt(b.description.match(/complexity of (\d+)/)[1], 10);
